...
void TileSheetEditor::loadTilesheet(const QString &imagePath)
{
    m_tilesheet.load(imagePath);
...
void TileSheetEditor::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        ...
        if (tileX >= 0 && tileY >= 0 && 
            tileX < m_tilesheet.width() / m_tileWidth &&
            tileY < m_tilesheet.height() / m_tileHeight) {
            
            m_selectedTilePos = QPoint(tileX, tileY);
            emit tileSelected(getSelectedTile());
            update();
            
            QDrag *drag = new QDrag(this);
            QMimeData *mimeData = new QMimeData;
            mimeData->setImageData(getSelectedTile());
