...
public:
    ...
    void end(/* std::shared_ptr<GameSnapshot> o std::shared_ptr<GameFinishedDTO> */) {
        // send_q.push(); // asegura push de update final
        send_q.close();
        sndr.join();
        // con.destroy_socket();
        recv_q.pop(); // desbloquear receiver por si estaba en push con queue llena
        rcvr.join();
        _is_finished = true;
    }
    ...
    ClientSession(ClientSession&&) = delete; // Thread subclasses (sndr, rcvr) can't be moved
